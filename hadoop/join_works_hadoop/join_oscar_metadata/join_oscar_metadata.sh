...
hadoop jar $HADOOP_INSTALL/share/hadoop/tools/lib/hadoop-streaming* \
 ...
 -io mongodb\
 -mapper ~/join_works_hadoop/join_oscar_metadata/mymapper_join.py\
 -reducer ~/join_works_hadoop/join_oscar_metadata/myreducer_join.py
...
echo ## Explode oscar nomination
mongo movie_dataset --eval 'db.oscar_nominations_out.drop()'
mongo movie_dataset --eval 'db.oscar_metadata_join_out.aggregate([{$unwind:{path:"$oscar_nominations"}},{$set:{"_id":"$oscar_nominations._id"}},{$project:{"oscar_nominations._id": 0 , "movie_id": 0}},{$out:{db:"movie_dataset",coll:"oscar_nominations_out"}}])'
